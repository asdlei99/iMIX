...
from ..stream import ItemFeature
from ..utils.phoc import phoc

TEXTVQA_PATH_CONFIG = yaml.load(
    open('datasets/configs/dataset_textvqa.yaml'))['dataset_configs']
...
class TextVQAReader(BaseDataReader):
  ...
  img_id, img_width, img_height, question, answers, valid_answers, img_feats,
  img_boxes, ocr_tokens, ocr_feats, ocr_boxes, ocr_fasttext, ocr_phoc
  """
  ...
  def __init__(self, splits: str, save_img_cache=True):
    ...
    self.ocr_dirs = [
        TEXTVQA_PATH_CONFIG['ocr_dir'][split][0] for split in self.splits
    ]
    self.img_dirs = [
        TEXTVQA_PATH_CONFIG['img_dir'][split][0] for split in self.splits
    ]
    self.frcnn_featrure_dirs = [
        TEXTVQA_PATH_CONFIG['frcnn_featrure'][split][0] for split in self.splits
    ]
    self.annotations_pathes = [
        TEXTVQA_PATH_CONFIG['annotations'][split][0] for split in self.splits
    ]
    ...
    self.feat_keys = [
        'img_feats', 'img_boxes', 'ocr_tokens', 'ocr_feats', 'ocr_boxes',
        'ocr_phoc', 'ocr_fasttext'
    ]
  ...
  def complete_info(self, item_feature):
    ...
    # load image features and bounding boxes
    img_feats = np.load(
        os.path.join(self.frcnn_featrure_dirs[split_idx], img_id + '.npy'))
    img_feats_info = np.load(
        os.path.join(self.frcnn_featrure_dirs[split_idx], img_id + '_info.npy'),
        allow_pickle=True)[()]
    ...
    ocr_feats_info = np.load(
        os.path.join(self.ocr_dirs[split_idx], img_id + '_info.npy'),
        allow_pickle=True)[()]
  ...
  def get_token_feat(self, tokens, switch):
    ...
    return np.array(
        [self.get_single_token_feat(token, switch) for token in tokens])
  ...
  def get_single_token_feat(self, token, switch):
    ...
    if switch == 'fasttext':
      return np.mean(
          np.array([self.get_fasttext_single_word(word) for word in words]), 0)
    elif switch == 'glove':
      return np.mean(
          np.array([self.get_glove_single_word(word) for word in words]), 0)

  def load_fasttext_weights(self):
    fin = io.open(
        self.fasttext_weights_path,
        'r',
        encoding='utf-8',
        newline='\n',
        errors='ignore')
