...
def launch(run_fn,
    ...
    if torch.cuda.is_available() is False:
        raise RuntimeError(
            'cuda is not available,please check if it is installed!')
    ...
    if dist_world_size > 1:
        try:
            ...
            spawn_fn_args = (run_fn, dist_world_size, nproc_per_node,
                             node_rank, dist_url, run_fn_args)
            spawn(
...
def is_right_dist_url(dist_url):
    if dist_url.startswith('tcp://') is False:
        raise Exception(
            'dist_url dose not support the format{},please use {}'.format(
                dist_url, 'tcp://'))


def _distributed_main(local_rank, run_fn, dist_world_size, nproc_per_node,
                      node_rank, dist_url, run_fn_args):
    if nproc_per_node > torch.cuda.device_count():
        raise RuntimeError(
            'The number of GPUs per machine is greater than the number of  GPUs available'
        )

    if dist_info._LOCAL_PROCESS_GROUP is not None:
        raise RuntimeError(
            '_LOCAL_PROCESS_GROUP is {},unable to set up local process group'.
            format(dist_info._LOCAL_PROCESS_GROUP))
    ...
    try:
        ...
        for idx in range(machine_nums):
            ranks_list = list(
                range(idx * nproc_per_node, (idx + 1) * nproc_per_node))
    ...
    def __getattr__(self, name):
        if self.has(name):
            ...
            raise AttributeError(
                'there is no such {} in the current pytorch environment variable'
                .format(name))
...
def set_pytorch_env_var(master_addr,
                        master_port,
                        world_size,
                        omp_num_threads=1):
