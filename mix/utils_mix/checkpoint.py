...
class IncompatibleKeys:
  ...
  def __log_missing_keys(self):
    if self.missing_keys:
      ...
      msg += '\n'.join(
          '  ' + k + self.list_to_str(v) for k, v in missing_keys_info.items())
      self.logger.warning(msg)
  ...
  def __log_unexpected_keys(self):
    if self.unexpected_keys:
      ...
      msg += '\n'.join(
          ' ' + k + self.list_to_str(v) for k, v in unexpected_keys)
      self.logger.warning(msg)
  ...
  def __remove_reused_missing_keys(self) -> List[str]:
    ...
    for names in param_map_of_names:
      if not all(name in unique_keys for name in names) and any(
          name in unique_keys for name in names):
        for name in names:
  ...
  @staticmethod
  def __named_modules_with_duplicated(model: Module,
                                      model_prefix: str = ''
  ...
  def remove_first_from_checkpoint(self):
    if len(self._checkpoint_files) > 0:
      ...
      if not remove_file_name.endswith(
        ...
        self.logger.info(
            'Removing {} from checkpoint_files'.format(remove_file_name))
...
class PeriodicCheckpointer:
  ...
  def __init__(self,
    ...
    assert isinstance(checkpointer, Checkpointer), TypeError(
        'checkpointer is {} type,but the desired type is Checkpointer'.format(
            type(checkpointer)))
