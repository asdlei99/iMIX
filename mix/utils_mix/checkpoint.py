...
class IncompatibleKeys:
    ...
    def __init__(self, model: Module, missing_keys: List[str],
                 unexpected_keys: List[str],
                 incorrect_shapes: List[_IncorrectShape]):
        self._model = model
    ...
    def __log_incorrect_shapes(self):
        for s in self.incorrect_shapes:
            key_msg = 'Due to model matching issues,some parameters {} will be skipped'.format(
                s.key)
            ck_shape_msg = 'checkpoint_model_shapes:{}'.format(
                s.checkpoint_model_shape)
    ...
    def __log_missing_keys(self):
        if self.missing_keys:
            ...
            msg += '\n'.join('  ' + k + self.list_to_str(v)
                             for k, v in missing_keys_info.items())
            self.logger.warning(msg)
    ...
    @staticmethod
    def __named_modules_with_duplicated(
            model: Module,
            model_prefix: str = '') -> Iterable[Tuple[str, Module]]:
        yield model_prefix, model
        for name, module in model._modules.items():
            if module is None:
                continue
            submodule_prefix = model_prefix + ('.'
                                               if model_prefix else '') + name
            yield from IncompatibleKeys.__named_modules_with_duplicated(
                model=module, model_prefix=submodule_prefix)
...
class Checkpointer:
    ...
    def remove_first_from_checkpoint(self):
        if len(self._checkpoint_files) > 0:
            ...
            if not remove_file_name.endswith(
                    'model_final.pth') and PathManager.exists(
                        osp.join(self.save_dir, remove_file_path)):
                PathManager.rm(remove_file_path)
                self.logger.info('Removing {} from checkpoint_files'.format(
                    remove_file_name))
    ...
    def load_checkpoint(
            self,
            path: str,
            specify_load_keys: Optional[List[str]] = None) -> object:
        if not path:
        ...
        if not osp.isfile(path):
            ...
            if not osp.isfile(path):
                raise ValueError(
                    'path is {}, no checkpoint found'.format(path))
            ...
            if key in model_state_dict:
                ...
                if model_shape != ck_model_shape:
                    incorrect_shapes.append(
                        _IncorrectShape(key, ck_model_shape, model_shape))
                    ck_model_state_dict.pop(key)

        incompatible = self.model.load_state_dict(
            ck_model_state_dict, strict=False)
    ...
    def _state_dict_to_tensor(state_dict: Dict[str, Any]) -> None:
        for k, v in state_dict.items():
            assert isinstance(v, (np.ndarray, torch.Tensor)), ValueError(
                '{}:{} is an unsupported type in checkpoint'.format(
                    k, type(v)))
            if isinstance(v, np.ndarray):
                state_dict[k] = torch.from_numpy(v)

    @staticmethod
    def __remove_prefix_from_state_dict(state_dict: Dict[str, Any],
                                        prefix: str) -> None:
...
class PeriodicCheckpointer:
    ...
    def __init__(self,
        ...
        assert isinstance(checkpointer, Checkpointer), TypeError(
            'checkpointer is {} type,but the desired type is Checkpointer'.
            format(type(checkpointer)))
        ...
        if max_epoch is not None:
            assert max_epoch > 0, ValueError(
                'max_epoch{} < 0'.format(max_epoch))
    ...
    def record_iter_checkpoint(self, iteration: int, **kwargs: Any) -> None:
        ...
        if (iteration + 1) % self.iter_period == 0:
            ...
            if self.max_num_checkpoints is not None and len(
                    self.checkpointer.checkpoint_files
