...
def default_argument_parser(
  ...
  parser.add_argument(
      '--build-submit', action='store_true', help='generate submission results')
  parser.add_argument(
      '--nproc_per_node',
      type=int,
      default=1,
  ...
  parser.add_argument(
      ...
      type=int,
      default=0,
      help='the rank of current node(unique per machine)')
  ...
  parser.add_argument(
      ...
      type=str,
      help='the IP address of mast node(rank 0)')
...
def seed_all_rng(seed=None):  # TODO(jinliang): rename set_all_rng_seed
  ...
  np.random.seed(seed + 1)  ###test forward TODO zhangrunze
  random.seed(seed)
  torch.set_rng_state(torch.manual_seed(seed + 2).get_state())
