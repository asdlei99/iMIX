...
class ImageFeatureEmbedding(nn.Module):
    ...
    def forward(self,
        ...
        tmp_embedding = torch.bmm(att_reshape,
                                  image_feat_variable)  # N x n_att x image_dim
...
class TopDownAttention(nn.Module):
    ...
    def _mask_attentions(attention, image_locs):
        ...
        tmp1[:num_loc] = torch.arange(
            0, num_loc, dtype=attention.dtype).unsqueeze(dim=0)
    ...
    def forward(self, image_feat, question_embedding, image_locs=None):
        ...
        if self.normalization.lower() == 'softmax':
            ...
            if image_locs is not None:
                ...
                masked_attention_sum = torch.sum(
                    masked_attention, dim=1, keepdim=True)
                masked_attention_sum += masked_attention_sum.eq(
                    0).float() + self.EPS
