...
class ReSC_BACKBONE(nn.Module):
  ...
  def __init__(self,
    ...
    ## Mapping module
    self.mapping_visu = ConvBatchNormReLU(
        512 if self.convlstm else 256, emb_size, 1, 1, 0, 1, leaky=leaky)

    self.mapping_lang = torch.nn.Sequential(
        nn.Linear(self.textdim, emb_size), nn.ReLU(), nn.Dropout(jemb_drop_out),
    ...
    if self.mstage:
      ...
      for n in range(0, NFilm):
        modules['out%d' % n] = torch.nn.Sequential(
            ConvBatchNormReLU(
                output_emb, output_emb // 2, 1, 1, 0, 1, leaky=leaky),
  ...
  def forward(self, raw_fword, raw_fvisu, word_mask):
    ...
    fword = Variable(
        torch.zeros(raw_fword.shape[0], raw_fword.shape[1],
                    self.emb_size).cuda())
    batch_size = raw_fword.size(0)

    for ii in range(raw_fword.shape[0]):
      ntoken = (word_mask[ii] != 0).sum()
      fword[ii, :ntoken, :] = F.normalize(
          self.mapping_lang(raw_fword[ii, :ntoken, :]), p=2, dim=1)
      ## [CLS], [SEP]
    ...
    x, attnscore_list = self.film(
        raw_fvisu, raw_fword, coord, fsent=None, word_mask=word_mask)
    if self.mstage:
      outbox = []
      ...
      x = torch.stack(
          x, dim=1).view(batch_size, -1, raw_fvisu.size(2), raw_fvisu.size(3))
...
class FiLMedConvBlock_context(nn.Module):
  ...
  def __init__(self,
    ...
    if self.baseline:
      self.fusion_layer = ConvBatchNormReLU(visudim + textdim + 8, emb_size, 1,
                                            1, 0, 1)
    else:
  ...
  def forward(self,
    ...
    if self.cont_map and fsent is not None:
      ...
      fcont = torch.matmul(context_score.view(B, 1, N),
                           fword.permute(0, 2, 1)).squeeze(1)
      fcontext = F.relu(self.context_map(fsent * fcont)).unsqueeze(2).repeat(
          1, 1, N)
      ## word attention
      tile_visu = torch.mean(
          fvisu.view(B, Dvisu, -1), dim=2, keepdim=True).repeat(1, 1, N)
      if self.fusion == 'cat':
    ...
    if self.baseline:
      ...
      film_param = film_param.view(B, 2 * self.emb_size, 1,
                                   1).repeat(1, 1, H, W)
...
class FiLMedConvBlock_multihop(nn.Module):
  ...
  def forward(self,
    ...
    for n in range(1, self.NFilm):
      score_list = [
          mask_softmax(score.squeeze(2).squeeze(2), word_mask, lstm=self.lstm)
          for score in attnscore_list
      ]

      score = torch.clamp(
          torch.max(torch.stack(score_list, dim=1), dim=1, keepdim=False)[0],
          min=0.,
          max=1.)
      ...
      attnscore_list.append(attn_score.view(
          B, N, 1, 1))  ## format match div loss in main func
      if self.intmd:
        intmd_feat.append(x)
      elif n == self.NFilm - 1:
        intmd_feat = [x]
