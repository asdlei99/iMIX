...
import torch
from mix.models.embedding.textembedding import AttnPool1d, MovieMcanMultiHeadAttention
...
class SelfGuidedAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_attn: int, dropout: float):
        ...
        self.multi_head_attn = nn.ModuleList([
            MovieMcanMultiHeadAttention(dim, num_attn, dropout=0.1)
            for _ in range(2)
        ])
...
class Modulation(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = (
            nn.Conv2d(num_features, 256, kernel_size=1) if compressed else
...
class MovieBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(self.width, self.width, stride_3x3, groups,
                             dilation)
