...
class CAGRAPH_BACKBONE(nn.Module):
    ...
    def forward(self, ques_feat, his_feat, rcnn_feat, ques_emb, rnd):
        ...
        his_atten_weight = F.softmax(
            self.Wa_1(
                F.dropout(atten_emb_1, self.d, training=self.training).view(
                    -1, self.nhid)).view(-1, rnd))
        h_emb = torch.bmm(
            his_atten_weight.view(-1, 1, rnd),
        ...
        ####################### question command #############################
        q_norm = F.normalize(
            self.ref_att(ques_feat.transpose(0, 1)), p=2, dim=-1)
        at = F.softmax(
            self.Wqt(
                F.dropout(q_norm, self.d,
                          training=self.training).view(-1,
                                                       self.nhid)).view(-1, L))
        ...
        sum_mes = self._create_neighbourhood(mes, belief_mat,
                                             self.neighbourhood_size)
        context_1 = self.W8(
            torch.cat((h_emb.expand_as(rcnn_feat), sum_mes), dim=2))
        graph2 = torch.cat((rcnn_feat, context_1), dim=2)

        ####################### T == 2 #######################################
        ####################### question command #############################
        q_norm2 = F.normalize(
            self.ref_att2(ques_feat.transpose(0, 1)), p=2, dim=-1)
        at2 = F.softmax(
            self.Wqt2(
                F.dropout(q_norm2, self.d,
                          training=self.training).view(-1,
                                                       self.nhid)).view(-1, L))
        q_c2 = torch.bmm(at2.view(-1, 1, L), ques_emb.transpose(0,
                                                                1)).squeeze(1)
        ####################### belief_matrix #############################
        ...
        sum_mes2 = self._create_neighbourhood(mes2, belief_mat2,
                                              self.neighbourhood_size)
        ...
        ####################### question command #############################
        q_norm3 = F.normalize(
            self.ref_att3(ques_feat.transpose(0, 1)), p=2, dim=-1)
        at3 = F.softmax(
            self.Wqt3(
                F.dropout(q_norm3, self.d,
                          training=self.training).view(-1,
                                                       self.nhid)).view(-1, L))
        q_c3 = torch.bmm(at3.view(-1, 1, L), ques_emb.transpose(0,
                                                                1)).squeeze(1)
        ####################### belief_matrix #############################
        ...
        sum_mes3 = self._create_neighbourhood(mes3, belief_mat3,
                                              self.neighbourhood_size)
        ...
        graph_att = F.softmax(
            self.W11(
                F.dropout(att_gq_emb, self.d, training=self.training).view(
                    -1, self.nhid)).view(-1, 36)).unsqueeze(1)
        graph_emb = torch.bmm(graph_att, graph4)

        #################### Multi-modal Fusion ############################
        concat_feat = torch.cat(
            (graph_emb.view(-1, 2 * self.nhid), ques_feat[-1].view(
                -1, self.nhid), h_emb.view(-1, self.nhid)), 1)
        final_feat = F.tanh(
            self.fc1(F.dropout(concat_feat, self.d, training=self.training)))
    ...
    def _create_neighbourhood_mes(self, message, top_ind):
        ...
        idx = top_ind.unsqueeze(-1).expand(batch_size, K, neighbourhood_size,
                                           feat_dim)
        return torch.gather(message, dim=2, index=idx)

    def _create_neighbourhood(self, message, belief_matrix,
                              neighbourhood_size):
        ...
        top_k, top_ind = torch.topk(
            belief_matrix, k=neighbourhood_size, dim=-1, sorted=False)
        top_k = torch.stack([F.softmax(top_k[:, k])
                             for k in range(K)]).transpose(
                                 0, 1)  # (batch_size, K, neighbourhood_size)
