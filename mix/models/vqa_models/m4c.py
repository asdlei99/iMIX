...
class M4C(nn.Module):
    ...
    def __init__(self, hidden_dim, dropout_prob, ocr_in_dim, encoder, backbone,
                 combine_model, head):
        super().__init__()
        ...
        self.linear_obj_feat_to_mmt_in = nn.Linear(
            self.encoder_model[1].out_dim, hidden_dim)

        # object location feature: relative bounding box coordinates (4-dim)
        self.linear_obj_bbox_to_mmt_in = nn.Linear(4, hidden_dim)

        self.obj_feat_layer_norm = BertLayerNorm(hidden_dim)
        self.obj_bbox_layer_norm = BertLayerNorm(hidden_dim)
        ...
        self.ocr_feat_layer_norm = BertLayerNorm(hidden_dim)
        self.ocr_bbox_layer_norm = BertLayerNorm(hidden_dim)
    ...
    def _forward_txt_encoding(self, data, fwd_results):
        fwd_results['txt_inds'] = data['text']
        text_len = data['text'].shape[1] * torch.ones(data['text'].shape[0])
        # binary mask of valid text (question words) vs padding
        fwd_results['txt_mask'] = _get_mask(text_len, data['text'].size(1))
    ...
    def _forward_ocr_encoding(self, data, fwd_results):
        ...
        ocr_order_vectors = torch.zeros_like(data['order_vectors'])
    ...
    def _forward_output(self, sample_list, fwd_results):
        ...
        dynamic_ocr_scores = self.ocr_ptr_net(mmt_dec_output, mmt_ocr_output,
                                              ocr_mask)
    ...
    def forward(self, data):
        ...
        self._forward_output(data, fwd_results)

        return fwd_results['scores']
