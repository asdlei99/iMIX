...
class M4C(BaseModel):
  ...
  def __init__(self, hidden_dim, dropout_prob, ocr_in_dim, encoder, backbone,
    ...
    self.linear_obj_feat_to_mmt_in = nn.Linear(self.encoder_model[1].out_dim,
                                               hidden_dim)
  ...
  def _forward_txt_encoding(self, data, fwd_results):
    ...
    text_len = data['input_ids'].shape[1] * torch.ones(
  ...
  def _forward_obj_encoding(self, data, fwd_results):
    ...
    obj_mmt_in = self.obj_feat_layer_norm(
        self.linear_obj_feat_to_mmt_in(obj_feat)) + self.obj_bbox_layer_norm(
  ...
  def _forward_ocr_encoding(self, data, fwd_results):
    ...
    ocr_feat = torch.cat([ocr_fasttext, ocr_phoc, ocr_fc7, ocr_order_vectors],
                         dim=-1)
    ...
    ocr_mmt_in = self.ocr_feat_layer_norm(
        self.linear_ocr_feat_to_mmt_in(ocr_feat)) + self.ocr_bbox_layer_norm(
