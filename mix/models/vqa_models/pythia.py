...
class PYTHIA(nn.Module):
    ...
    def __init__(self, embedding, encoder, backbone, combine_model, head):
        ...
        self.combine_model = build_combine_layer(
            combine_model)  ###combine text and image
    ...
    def process_feature_embedding(self,
        ...
        feature_dim = data['img_feature1'].shape[1] * torch.ones(
            data['img_feature1'].shape[0])
    ...
    def process_context_feature_embedding(self,
                                          data,
                                          text_embedding_total,
                                          order_vectors=None,
                                          batch_size_t=None):
        ...
        feature_dim = data['context_feature1'].shape[1] * torch.ones(
            data['context_feature1'].shape[0])
        # Now, iterate to get final attended image features
    ...
    def forward(self, data):
        text = self.embedding_model[0](data['text'])
