...
class ReSC(BaseModel):
  ...
  def forward_train(self, data):
    ...
    outbox, attnscore_list = self.backbone(raw_fword, raw_fvisu, input_mask)
    bbox = data['bbox'].cuda()
    anchors_full = self.anchors_full

    # losses = self.compute_loss(attnscore_list, bbox, anchors_full,
    #                            attnscore_list, input_mask)
    return outbox, attnscore_list
