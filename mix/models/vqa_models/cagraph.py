...
class CAGRAPH(nn.Module):
    ...
    def __init__(self, embedding, encoder, backbone, batch_size):
        ...
        self.backbone = build_backbone(
            backbone)  ###包括two_branch_embedding(MCAN), BERT
        ...
        self.wrong_hidden = self.backbone.init_hidden(
            batch_size)  ## question dataset
        self.nhid = 512
        self.img_embed = nn.Linear(2048, self.nhid)

    def forward_rnd(self, ques_emb, his_emb, img, ques_hidden, his_hidden,
                    rnd):

        b = img.size(0)
        L = ques_emb.size(0)
        r_feat = img.contiguous().view(-1, 36, 2048)
        r_feat = r_feat / (r_feat.norm(p=2, dim=2, keepdim=True) +
                           1e-12).expand_as(r_feat)
        ...
        his_feat = his_feat[-1]
        final_feat = self.backbone(ques_feat1, his_feat, rcnn_feat, ques_emb,
                                   rnd)
    ...
    def forward(self, data):
        ...
        for rnd in range(10):
            ...
            ques = question[:, rnd, :].t()
            his = history[:, :rnd + 1, :].clone().view(-1,
                                                       24).t()  # his_length=24

            ans = answer[:, rnd, :].t()
            tans = answerT[:, rnd, :].t()
            wrong_ans = opt_answerT[:,
                                    rnd, :].clone().view(-1,
                                                         9).t()  # ans_length=9
            ...
            batch_sample_idx.resize_(batch_size,
                                     30).zero_()  # neg_batch_sample=30
            sample_batch_neg(answerIdx[:, rnd], opt_answerIdx[:, rnd, :],
                             batch_sample_idx, 30)  # neg_batch_sample=30
            ...
            self.his_hidden = repackage_hidden(self.his_hidden,
                                               his_input.size(1))

            featD, ques_hidden = self.forward_rnd(ques_emb, his_emb, img_input,
                                                  self.ques_hidden,
                                                  self.his_hidden, rnd + 1)

            self.ques_hidden = tuple([x.detach() for x in ques_hidden])

            ans_real_emb = self.embedding_model(ans_target, format='index')
            ans_wrong_emb = self.embedding_model(
                wrong_ans_input, format='index')
            self.real_hidden = repackage_hidden(self.real_hidden, batch_size)
            self.wrong_hidden = repackage_hidden(self.wrong_hidden,
                                                 ans_wrong_emb.size(1))
            real_feat = self.encoder[-1](ans_real_emb, ans_target,
                                         self.real_hidden)
            wrong_feat = self.encoder[-1](ans_wrong_emb, wrong_ans_input,
                                          self.wrong_hidden)
