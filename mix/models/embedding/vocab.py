...
class BaseVocab:
  ...
  def __init__(self,
    ...
    scratch based on a custom vocab. This will initialize the random vectors
    for the vocabulary you pass. Get the vectors using `get_vectors` function.
    This will also create random embeddings for.
...
class IntersectedVocab(BaseVocab):
  ...
  def __init__(self, vocab_file, embedding_name, *args, **kwargs):
    ...
    self.vectors = torch.empty((self.get_size(), len(embedding.vectors[0])),
                               dtype=torch.float)
