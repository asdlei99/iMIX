...
class TextEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        if emb_type == 'identity':
            ...
            self.module = AttentionTextEmbedding(hidden_dim, num_layers,
                                                 conv1_out, conv2_out, dropout,
                                                 embedding_dim, kernel_size,
                                                 padding, **kwargs)
            self.text_out_dim = self.module.text_out_dim

        elif emb_type == 'mcan':
            self.module = SAEmbedding(hidden_dim, embedding_dim, num_layers,
                                      dropout, **kwargs)
...
class _netW(nn.Module):
    ...
    def forward(self, input, format='index'):
        if format == 'onehot':
            ...
            out = F.dropout(
                self.word_embed(input), self.d, training=self.training)
    ...
    def init_pretrained_wemb(self, emb_dim):
        ...
        f = json.load(
            open('/home/datasets/visual-diag/visdial_params.json', 'r'))
        self.itow = f['itow']
        ...
        embedding_mat = np.zeros((self.vocab_size + 1, emb_dim),
                                 dtype=np.float32)
...
class AttentionTextEmbedding(nn.Module):
    ...
    def __init__(self, hidden_dim, num_layers, conv1_out, conv2_out, dropout,
                 embedding_dim, kernel_size, padding, **kwargs):
        super().__init__()
...
class MovieMcanMultiHeadAttention(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
        ...
        x, self.p_attn = self.qkv_attention(
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_attn: int, dropout: float):
        super().__init__()
        self.multi_head_attn = MovieMcanMultiHeadAttention(
            dim, num_attn, dropout=0.1)
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor) -> torch.Tensor:
        x = self.ln_mha(x +
                        self.drop_mha(self.multi_head_attn(x, x, x, x_mask)))
