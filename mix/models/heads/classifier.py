...
class ClassifierLayer(nn.Module):
    ...
    def __init__(self, classifier_type, in_dim, out_dim, **kwargs):
        ...
        if classifier_type == 'weight_norm':
            ...
            self.module = BertClassifierHead(in_dim, out_dim,
                                             kwargs.get('config', None)).module
...
class LogitClassifier(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, **kwargs):
        ...
        if 'pretrained_image' in kwargs and kwargs[
                'pretrained_text'] is not None:
            self.linear_text.weight.data.copy_(
                torch.from_numpy(kwargs['pretrained_text']))

        if 'pretrained_image' in kwargs and kwargs[
                'pretrained_image'] is not None:
...
class LCGNClassifer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, OUT_QUESTION_MUL, CMD_DIM,
                 outputDropout):
        super().__init__()
        ...
        self.classifier_layer = nn.Sequential(
            nn.Dropout(1 - outputDropout), Linear(in_dim, CMD_DIM), nn.ELU(),
            nn.Dropout(1 - outputDropout), Linear(CMD_DIM, out_dim))
