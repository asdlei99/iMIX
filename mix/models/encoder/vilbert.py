...
from transformers.modeling_bert import (ACT2FN, BertConfig, BertEmbeddings,
                                        BertEncoder, BertIntermediate,
                                        BertLayer, BertLayerNorm, BertOutput,
                                        BertPooler,
                                        BertPredictionHeadTransform,
                                        BertPreTrainedModel)

from mix.models.embedding import (BertImageFeatureEmbeddings,
...
class BertImageSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_head_size = int(config.v_hidden_size /
...
class BertImageIntermediate(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.v_hidden_size,
...
class BertImageOutput(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.v_intermediate_size,
...
class BertBiAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores1 = torch.matmul(query_layer2,
                                         key_layer1.transpose(-1, -2))
        attention_scores1 = attention_scores1 / math.sqrt(
            self.attention_head_size)
        ...
        attention_scores2 = torch.matmul(query_layer1,
                                         key_layer2.transpose(-1, -2))
        attention_scores2 = attention_scores2 / math.sqrt(
            self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel
        # forward() function)
        ...
        new_context_layer_shape2 = context_layer2.size()[:-2] + (
            self.all_head_size, )
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id,
            ...
            for idx in range(t_start, t_end):
                txt_embedding = self.layer[idx](txt_embedding,
                                                txt_attention_mask)
            ...
            if count == 0 and self.in_batch_pairs:
                ...
                image_embedding = (
                    image_embedding.unsqueeze(0).expand(
                        batch_size, batch_size, num_regions,
                        v_hidden_size).contiguous().view(
                            batch_size * batch_size, num_regions,
                            v_hidden_size))
                image_attention_mask = (
                    image_attention_mask.unsqueeze(0).expand(
                        batch_size, batch_size, 1, 1,
                        num_regions).contiguous().view(batch_size * batch_size,
                                                       1, 1, num_regions))

                txt_embedding = (
                    txt_embedding.unsqueeze(1).expand(
                        batch_size, batch_size, num_words,
                        t_hidden_size).contiguous().view(
                            batch_size * batch_size, num_words, t_hidden_size))
                txt_attention_mask = (
                    txt_attention_mask.unsqueeze(1).expand(
                        batch_size, batch_size, 1, 1,
                        num_words).contiguous().view(batch_size * batch_size,
                                                     1, 1, num_words))
                co_attention_mask = (
                    co_attention_mask.unsqueeze(1).expand(
    ...
    def forward(
        ...
        if image_attention_mask is None:
            image_attention_mask = torch.ones(
                ...
                image_feature.size(1)).type_as(input_txt)

        if self.task_specific_tokens:
            # extend the mask
            mask_tokens = input_txt.new().resize_(input_txt.size(0),
                                                  1).fill_(1)
        ...
        extended_image_attention_mask = image_attention_mask.unsqueeze(
            1).unsqueeze(2)
        ...
        extended_image_attention_mask = (
            1.0 - extended_image_attention_mask) * -10000.0

        if co_attention_mask is None:
            co_attention_mask = torch.zeros(
                input_txt.size(0), image_feature.size(1),
...
class ViLBERTForPretraining(nn.Module):
    ...
    def tie_weights(self):
        ...
        them instead.
        """
        self.bert._tie_or_clone_weights(self.cls.predictions.decoder,
    ...
    def __init__(self, **kwargs):
        ...
        if self.bert_model_name is None:
            ...
            self.bert = ViLBERTBase.from_pretrained(
                self.config['bert_model_name'],
                config=self.bert_config,
                cache_dir=os.path.join('/home/jinliang/.cache/torch',
