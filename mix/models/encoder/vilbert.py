...
import pickle
import math
from transformers.modeling_bert import (BertConfig, BertEmbeddings, BertEncoder,
                                        BertPreTrainedModel,
                                        BertPredictionHeadTransform, BertPooler,
                                        BertLayer, BertOutput, BertIntermediate,
                                        ACT2FN)
      ...
      if count == 0 and self.in_batch_pairs:
        ...
        image_embedding = (
            image_embedding.unsqueeze(0).expand(
                ...
                v_hidden_size).contiguous().view(batch_size * batch_size,
                                                 num_regions, v_hidden_size))
        image_attention_mask = (
            image_attention_mask.unsqueeze(0).expand(
                batch_size, batch_size, 1, 1,
                num_regions).contiguous().view(batch_size * batch_size, 1, 1,
                                               num_regions))
        ...
        txt_embedding = (
            txt_embedding.unsqueeze(1).expand(batch_size, batch_size, num_words,
                                              t_hidden_size).contiguous().view(
                                                  ...
                                                  num_words, t_hidden_size))
        txt_attention_mask = (
            txt_attention_mask.unsqueeze(1).expand(batch_size, batch_size, 1, 1,
        ...
        co_attention_mask = (
            co_attention_mask.unsqueeze(1).expand(
                ...
                num_words).contiguous().view(batch_size * batch_size, 1,
  ...
  def forward(
    ...
    if image_attention_mask is None:
      image_attention_mask = torch.ones(
          image_feature.size(0), image_feature.size(1)).type_as(input_txt)
    ...
    extended_image_attention_mask = image_attention_mask.unsqueeze(1).unsqueeze(
        2)
    ...
    extended_image_attention_mask = (1.0 -
                                     extended_image_attention_mask) * -10000.0
