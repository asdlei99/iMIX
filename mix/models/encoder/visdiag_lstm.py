...
class VisDialPrincipleLSTM(nn.Module):
    ...
    def __init__(self, word_embedding_size, lstm_hidden_size, lstm_num_layers,
                 dropout):
        super().__init__()
    ...
    def forward(self, ques_embed, ques_len, cap_emb, cap_len, his_embed,
                hist_len):
...
class VisDialANSEncoder(nn.Module):
    ...
    def forward(self, input_feat, idx, hidden):
        ...
        mask[idx.data ==
             self.vocab_size] = 1  # also set the last token to be 1
        ...
        atten = self.W2(
            F.dropout(
                F.tanh(self.W1(output.view(-1, self.nhid))),
                self.d,
...
class DynamicRNN(nn.Module):
    ...
    def forward(self, seq_input, seq_lens, initial_state=None):
        ...
        outputs = pad_packed_sequence(
            ...
            total_length=max_sequence_length)[0].index_select(
