...
def update_bn_stats(
    ...
    stats of the current model. In this function, the BN stats are recomputed
    with fixed weights, to make the running average more precise. Specifically,
    it computes the true average of per-batch mean/variance instead of the
    running average.
    ...
    running_var = [torch.zeros_like(bn.running_var)
                   for bn in bn_layers]  # pyre-ignore
