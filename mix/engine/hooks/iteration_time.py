...
class IterationTimerHook(HookBase):
    ...
    def after_train_iter(self):
        iter_done = self.trainer.iter - self.trainer.start_iter + 1
        if iter_done >= self._warmup_iter:
            iter_sec = Timer.passed_seconds(self._iter_start_time, Timer.now())
            self.trainer.log_buffer.put_scalars(iter_time=iter_sec)
    ...
    def _write_log(self):
        ...
        if self.trainer.by_epoch:
            ...
            logger.info(
                'The Whole training speed:{} epochs in {} ({} /epoch)'.format(
