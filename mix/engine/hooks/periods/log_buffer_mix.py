...
import torch
from mix.utils_mix.config import MIXEasyDict
# from mix.utils.history_buffer import HistoryBuffer
from mix.utils_mix.history_buffer import HistoryBuffer
...
class Histogram:
    ...
    def _convert(self, name, data, iter_idx):
        ht_min, ht_max, ht_counts = data.min().item(), data.max().item(
...
class TrainInfo:
    ...
    def has_name(name):
        return name in [
            'iter', 'epoch', 'epoch_inner_iter', 'single_epoch_iters',
            'is_by_epoch'
        ]
    ...
    def __setattr__(self, name, value):
        if self.has_name(name):
            ...
            raise AttributeError(
                'there is no such {} Attribute in TrainInfo'.format(name))


class VisualizeScalar:

    def __init__(self):
        self._scalars = MIXEasyDict()
        self._history = defaultdict(HistoryBuffer)
        self._smoothing_hints = MIXEasyDict()
    ...
    def has_name(name):
        ...
        return name in attr_list or VisualizeScalar._underline_prefix(
            name) in attr_list
    ...
    def add_single(self,
                   name,
                   data,
                   is_smooth_hint: bool = True,
                   iter_idx: int = 0):
        h = self._history[name]
        h.update(float(data), iter_idx)

        self._scalars[name] = data
        if self._smoothing_hints.get(name) is None:
            self._smoothing_hints[name] = is_smooth_hint
        else:
            assert ValueError(
                'Scalar {} should be add with  a different smoothing_hits'.
                format(name))
    ...
    def add_some(self,
        ...
        for k, v in kwargs.items():
            self.add_single(
                k, v, is_smooth_hint=is_smooth_hint, iter_idx=iter_idx)
...
def get_log_buffer():
    assert len(
        _CURRENT_LOG_BUFFER_STACK
    ), "get_log_buffer() has to be called inside a 'with LogBufferStorage(...)' context!"
...
class LogBufferStorage:
    ...
    def __init__(self, start_iter=0, sigle_epoch_iters=0, by_epoch=True):
        self.train_info = TrainInfo(
            iter=start_iter,
            single_epoch_iters=sigle_epoch_iters,
            is_by_epoch=by_epoch)
    ...
    def put_scalar(self, name, data, smoothing_hint=True):
        self.vis_scalars.add_single(name, data, smoothing_hint,
                                    self.train_info.iter)

    def put_scalars(self, *, smoothing_hint: bool = True, **kwargs):
        self.vis_scalars.add_some(
            is_smooth_hint=smoothing_hint,
            iter_idx=self.train_info.iter,
            **kwargs)
    ...
    def has_name(self, name):
        log_buffer_attr = [
            'train_info', 'vis_data', 'histograms', 'current_prefix',
            'vis_scalars'
        ]
