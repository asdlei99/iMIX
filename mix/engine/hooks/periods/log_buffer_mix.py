...
class TrainInfo:
  ...
  def has_name(name):
    return name in [
        'iter', 'epoch', 'epoch_inner_iter', 'single_epoch_iters', 'is_by_epoch'
    ]
...
class VisualizeScalar:
  ...
  def add_single(self,
    ...
    if self._smoothing_hints.get(name) is None:
      ...
      assert ValueError(
          'Scalar {} should be add with  a different smoothing_hits'.format(
              name))
...
class LogBufferStorage:
  ...
  def put_scalars(self, *, smoothing_hint: bool = True, **kwargs):
    self.vis_scalars.add_some(
        is_smooth_hint=smoothing_hint, iter_idx=self.train_info.iter, **kwargs)
  ...
  def has_name(self, name):
    log_buffer_attr = [
        'train_info', 'vis_data', 'histograms', 'current_prefix', 'vis_scalars'
    ]
