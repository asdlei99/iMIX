...
class LogBufferStorage:
  ...
  In the future we may add support for storing / logging other types of data if
  needed.
  """
  ...
  def latest_with_smoothing_hint(self, window_size=20):
    """Similar to :meth:`latest`, but the returned values are either the un-
    smoothed original latest value, or a median of the given window_size,
    ...
    for k, v in self._latest_scalars.items():
      result[k] = self._history[k].median(
          window_size) if self._smoothing_hints[k] else v
    return result
  ...
  def step(self):
    ...
    The storage will then be able to associate the new data with the correct
    iteration number.
    """
    self._iter += 1
    self._latest_scalars = {}
