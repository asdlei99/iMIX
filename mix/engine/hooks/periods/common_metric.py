...
class CommonMetricLoggerHook(LogBufferWriter):
    ...
    def __get_used_max_memory():
        if torch.cuda.is_available():
            max_used_memory = torch.cuda.max_memory_allocated(
            ) / 1024.0 / 1024.0
    ...
    def __get_by_iter_train_time(self):
        try:
            iter_time = self.log_buffer.history('iter_time').global_avg
            eta_time = self.log_buffer.history('iter_time').median(1000) * (
                self._max_iter - self.log_buffer.iter)
            ...
            iter_time_str = ''
            if self._recorder_iter_time is not None:
                estimate_iter_time = (Timer.now() -
                                      self._recorder_iter_time.time) / (
                                          self.log_buffer.iter -
                                          self._recorder_iter_time.iteration)
                eta_time = estimate_iter_time * (
                    self._max_iter - self.log_buffer.iter)
                eta_time_str = str(datetime.timedelta(seconds=int(eta_time)))
                iter_time_str = str(
                    datetime.timedelta(seconds=int(estimate_iter_time)))

            self._recorder_iter_time = _RecorderTime(self.log_buffer.iter,
                                                     Timer.now())
        finally:
    ...
    def __iter_metric(self):
        iteration = self.log_buffer.iter
