...
def to_tensor_and_normalize(image):
  return functional.normalize(
      functional.to_tensor(image),
      mean=(0.485, 0.456, 0.406),
      std=(0.229, 0.224, 0.225))


def letterbox(img, mask, height,
              color=(123.7, 116.3,
                     103.5)):  # resize a rectangular image to a padded square
  ...
  if mask is not None:
    mask = cv2.resize(
        mask, new_shape, interpolation=cv2.INTER_NEAREST)  # resized, no border
    mask = cv2.copyMakeBorder(
        mask, top, bottom, left, right, cv2.BORDER_CONSTANT,
        value=255)  # padded square
...
def wrap_points(targets, M, height, a):
  ...
  # warp points
  xy = np.ones((4, 3))
  xy[:, :2] = np.array(points)[[0, 1, 2, 3, 0, 3, 2,
                                1]].reshape(4, 2)  # x1y1, x2y2, x1y2, x2y1
  ...
  xy = np.concatenate(
      (x - w / 2, y - h / 2, x + w / 2, y + h / 2)).reshape(4, 1).T
