...
class BertField(SequenceField[Dict[str, torch.Tensor]]):
  ...
  def __init__(self,
    ...
    if len(self.tokens) != self.embs.shape[0]:
      raise ValueError(
          'The tokens you passed into the BERTField, {} '
          "aren't the same size as the embeddings of shape {}".format(
              self.tokens, self.embs.shape))
  ...
  @overrides
  def batch_tensors(
      self, tensor_list: List[Dict[str,
