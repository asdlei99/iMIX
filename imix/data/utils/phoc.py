...
with open(
    os.path.join(
        os.path.dirname(os.path.realpath(__file__)), 'bigrams_new.txt'),
    'r') as f:
  for line in f:
...
def build_phoc(words,
  ...
  """Calculate Pyramidal Histogram of Characters (PHOC) descriptor (see Almazan
  2014).
  ...
  if on_unknown_unigram not in ['error', 'warn']:
    raise ValueError('I don\'t know the on_unknown_unigram parameter \'%s\'' %
                     on_unknown_unigram)
    ...
    for index, char in enumerate(word):
      ...
      if char not in char_indices:
        if on_unknown_unigram == 'warn':
          logger.warn('The unigram \'%s\' is unknown, skipping this character',
                      char)
          continue
        else:
      ...
      for level in unigram_levels:
        for region in range(level):
          ...
          if size(overlap(char_occ, region_occ)) / size(char_occ) >= 0.5:
            feat_vec_index = sum([
                l for l in unigram_levels if l < level
