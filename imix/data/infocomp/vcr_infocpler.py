...
class VCRInfoCpler(BaseInfoCpler):
  ...
  def complete_info(self, item_feature: ItemFeature):
    questions_embeddings, questions_masks = self.get_pad_ndarray(
        [s.embs for s in item_feature.question.field_list],
        length=self.max_ques_length,
        need_mask=True,
        lowers=False)
    questions_tokens = [
        list(map(str, s.tokens)) for s in item_feature.question.field_list
    ]  # [s.tokens for s in item_feature.question.field_list]
    questions_obj_tags = self.get_pad_2dlst(
        [s.labels for s in item_feature.question_tags.field_list],
        length=self.max_ques_length,
        dtype=np.int,
        lowers=True)
    answers_embeddings, answers_masks = self.get_pad_ndarray(
        [s.embs for s in item_feature.answers.field_list],
        length=self.max_answer_length,
        need_mask=True,
        lowers=False)
    answers_tokens = [
        list(map(str, s.tokens)) for s in item_feature.answers.field_list
    ]  #[s.tokens for s in item_feature.answers.field_list]
    answers_obj_tags = self.get_pad_2dlst(
        [s.labels for s in item_feature.answer_tags.field_list],
        length=self.max_answer_length,
        dtype=np.int,
        lowers=True)
    label = item_feature.label.label
    meta_data = item_feature.metadata
    segms = self.get_pad_segm(
        item_feature.segms.array, length=self.max_obj_length, lowers=True)
    objects = self.get_pad_1darray(
        [s.label for s in item_feature.objects.field_list],
        length=self.max_obj_length,
        dtype=np.int,
        lowers=True)
    boxes = self.get_pad_2darray(
        item_feature.boxes.array,
        length=self.max_obj_length,
        dtype=np.float64,
        lowers=False)
    image = item_feature.image
    question_max_num = np.array(
        [len(s.embs) for s in item_feature.question.field_list]).max()
  ...
  def get_pad_ndarray(self, lst, length=None, need_mask=False, lowers=False):
    ...
    arr_pad = -2 * np.ones(
        (n, length, dim), dtype=spl.dtype) if lowers else np.zeros(
  ...
  def get_pad_1darray(self, lst, length=None, dtype=np.int, lowers=False):
    ...
    if length is None:
      length = self.default_max_length
    arr_pad = -2 * np.ones((length,), dtype=dtype) if lowers else np.zeros(
        (length,), dtype=dtype)
    lt = min([length, arr.shape[0]])
    arr_pad[:lt] = arr[:lt]
    return arr_pad
  ...
  def get_pad_2darray(self, arr2d, length=None, dtype=None, lowers=False):
    ...
    if length is None:
      length = self.default_max_length
    arr_pad = -2 * np.ones((length, dim), dtype=dtype) if lowers else -np.ones(
        (length, dim), dtype=dtype)
  ...
  def get_pad_2dlst(self, lst, length=None, dtype=np.int, lowers=False):
    ...
    if length is None:
      length = self.default_max_length
    arr_pad = -2 * np.ones((n, length), dtype=dtype) if lowers else np.zeros(
        (n, length), dtype=dtype)
    for i, arr in enumerate(lst):
  ...
  def get_pad_segm(self, segm, length=None, lowers=False):
    ...
    if length is None:
      length = self.default_max_length
    segm_pad = -2 * np.ones(
        (length, size_0, size_1), dtype=segm.dtype) if lowers else np.zeros(
