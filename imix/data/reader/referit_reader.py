...
class ReferitReader(IMIXDataReader):
  ...
  def __init__(self, cfg):
    ...
    self.imfilelist = {
        split: cfg['annotations']['imlist'][split] for split in self.splits
    }
    self.querylist = {
        split: cfg['annotations']['query'][split] for split in self.splits
    }
  ...
  def augment(self, img, bbox, phrase):
    ...
    if self.augment:
      ...
      if self.augment_flip and random.random() > 0.5:
        ...
        phrase = [
            p.replace('right',
                      '*&^special^&*').replace('left', 'right').replace(
                          '*&^special^&*', 'left') for p in phrase
        ]
