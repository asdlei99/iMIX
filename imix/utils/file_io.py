...
def get_cache_dir(cache_dir: Optional[str] = None) -> str:
  """Returns a default directory to cache static files (usually downloaded from
  Internet), if None is provided.
...
class PathHandler:
  ...
  It routes I/O for a generic URI which may look like "protocol://*" or a
  canonical filepath "/foo/bar/baz".
  """
...
class HTTPURLHandler(PathHandler):
  ...
  def _get_local_path(self, path: str, **kwargs: Any) -> str:
    ...
    if path not in self.cache_map or not os.path.exists(self.cache_map[path]):
      ...
      dirname = os.path.join(get_cache_dir(),
...
class OneDrivePathHandler(PathHandler):
  ...
  def create_one_drive_direct_download(self, one_drive_url: str) -> str:
    """Converts a short OneDrive URI into a download link that can be used with
    wget.
    ...
    data_b64_string = (
        data_b64.decode('utf-8').replace('/', '_').replace('+',
                                                           '-').rstrip('='))
    result_url = (
...
class PathManagerBase:
  """A class for users to open generic paths or translate generic paths to file
  names.
  ...
  def copy(self,
    ...
    return self.__get_path_handler(src_path)._copy(src_path, dst_path,
                                                   overwrite, **kwargs)
  ...
  def symlink(self, src_path: str, dst_path: str, **kwargs: Any) -> bool:
    ...
    assert self.__get_path_handler(  # type: ignore
        src_path) == self.__get_path_handler(dst_path)
    return self.__get_path_handler(src_path)._symlink(src_path, dst_path,
                                                      **kwargs)
  ...
  def set_strict_kwargs_checking(self, enable: bool) -> None:
    """Toggles strict kwargs checking. If enabled, a ValueError is thrown if
    any unused parameters are passed to a PathHandler function. If disabled,
