...
class Checkpointer(object):
  ...
  def load(self,
    ...
    if (incompatible
        is not None):  # handle some existing subclasses that returns None
  ...
  def get_all_checkpoint_files(self) -> List[str]:
    ...
    all_model_checkpoints = [
        ...
        if PathManager.isfile(os.path.join(self.save_dir, file)) and
        file.endswith('.pth')
    ]
  ...
  def resume_or_load(self, path: str, *, resume: bool = True) -> object:
    ...
    checkpoint, if exists. Otherwise, load checkpoint from the given path. This
    is useful when restarting an interrupted training job.
  ...
  def _load_file(self, f: str) -> object:
    ...
    Can be overwritten by subclasses to support
    different formats.
    Args:
  ...
  def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
    ...
    if incompatible.missing_keys:
      missing_keys = _filter_reused_missing_keys(self.model,
  ...
  def _convert_ndarray_to_tensor(self, state_dict: Dict[str, Any]) -> None:
    ...
    for k in list(state_dict.keys()):
      ...
      if not isinstance(v, np.ndarray) and not isinstance(v, torch.Tensor):
        raise ValueError('Unsupported type found in checkpoint! {}: {}'.format(
            k, type(v)))
...
class PeriodicCheckpointer:
  ...
  When `.step(iteration)` is called, it will execute `checkpointer.save` on the
  given checkpointer, if iteration is a multiple of period or if `max_iter` is
  reached.
  """
    ...
    if (iteration + 1) % self.period == 0:
      ...
      if self.max_to_keep is not None:
        ...
        if len(self.recent_checkpoints) > self.max_to_keep:
          ...
          if PathManager.exists(file_to_delete) and not file_to_delete.endswith(
              'model_final.pth'):
  ...
  def save(self, name: str, **kwargs: Any) -> None:
    """Same argument as :meth:`Checkpointer.save`. Use this method to manually
    save checkpoints outside the schedule.
...
def _filter_reused_missing_keys(model: nn.Module, keys: List[str]) -> List[str]:
  """Filter "missing keys" to not include keys that have been loaded with
  another name."""
  keyset = set(keys)
...
def get_missing_parameters_message(keys: List[str]) -> str:
  """Get a logging-friendly message to report parameter names (keys) that are
  in the model but not found in a checkpoint.
...
def get_unexpected_parameters_message(keys: List[str]) -> str:
  """Get a logging-friendly message to report parameter names (keys) that are
  in the checkpoint but not found in the model.
...
def _group_checkpoint_keys(keys: List[str]) -> Dict[str, List[str]]:
  """Group keys based on common prefixes.

  A prefix is the string up to the final
  "." in each key.
  Args:
...
def _named_modules_with_dup(
    model: nn.Module, prefix: str = '') -> Iterable[Tuple[str, nn.Module]]:
