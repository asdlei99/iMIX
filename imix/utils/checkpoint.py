...
class Checkpointer(object):
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> object:
        """Load from the given checkpoint. When path points to network file,
        this function has to be called on all ranks.
    ...
    def get_all_checkpoint_files(self) -> List[str]:
        ...
        all_model_checkpoints = [
            os.path.join(self.save_dir, file) for file in PathManager.ls(self.save_dir)
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
        ...
        for k, shape_checkpoint, shape_model in incompatible.incorrect_shapes:
            self.logger.warning("Skip loading parameter '{}' to the model due to incompatible "
...
class PeriodicCheckpointer:
    ...
    When `.step(iteration)` is called, it will execute `checkpointer.save` on the given checkpointer, if iteration is a
        ...
        if (iteration + 1) % self.period == 0:
            ...
            if self.max_to_keep is not None:
                ...
                if len(self.recent_checkpoints) > self.max_to_keep:
                    ...
                    if PathManager.exists(file_to_delete) and not file_to_delete.endswith('model_final.pth'):
    ...
    def save(self, name: str, **kwargs: Any) -> None:
        """Same argument as :meth:`Checkpointer.save`. Use this method to
        manually save checkpoints outside the schedule.
