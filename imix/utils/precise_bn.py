...
def update_bn_stats(
  ...
  iteration, so the running average can not precisely reflect the actual stats
  of the current model. In this function, the BN stats are recomputed with
  fixed weights, to make the running average more precise. Specifically, it
  computes the true average of per-batch mean/variance instead of the running
  average.
...
def get_bn_modules(model: nn.Module) -> List[nn.Module]:
  ...
  fvcore.precise_bn.BN_MODULE_TYPES for a list of all modules that are included
  in this search.
