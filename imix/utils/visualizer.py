...
class Visualizer:
  ...
  def draw_instance_predictions(self, predictions):
    ...
    if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get(
      ...
      colors = [
          self._jitter([x / 255
                        for x in self.metadata.thing_colors[c]])
          for c in classes
      ]
      alpha = 0.8
    else:
  ...
  def overlay_instances(self,
    ...
    for i in range(num_instances):
      ...
      if labels is not None:
        ...
        if (instance_area < _SMALL_OBJECT_AREA_THRESH * self.output.scale or
            y1 - y0 < 40 * self.output.scale):
  ...
  def draw_and_connect_keypoints(self, keypoints):
    ...
    connections to draw lines between appropriate keypoints. This follows color
    heuristics for line color.
    ...
    try:
      ...
      if nose_x is not None:
        self.draw_line([nose_x, mid_shoulder_x], [nose_y, mid_shoulder_y],
                       color=_RED)
      ...
      try:
        ...
        self.draw_line([mid_hip_x, mid_shoulder_x], [mid_hip_y, mid_shoulder_y],
                       color=_RED)
    return self.output
  ...
  def draw_binary_mask(self,
    ...
    if text is not None and has_valid_segment:
      ...
      for cid in range(1, _num_cc):
        if cid == largest_component_id or stats[cid,
                                                -1] > _LARGE_MASK_AREA_THRESH:
  ...
  def _change_color_brightness(self, color, brightness_factor):
    """Depending on the brightness_factor, gives a lighter or darker color i.e.
    a color with less or more saturation than the original color.
  ...
  def _convert_masks(self, masks_or_polygons):
    """Convert different format of masks or polygons to a tuple of masks and
    polygons.
