...
def timeit(num_iters: int = -1, warmup_iters: int = 0) -> Callable[[], Callable[[], Dict[str, float]]]:
...
def benchmark(
    ...
    dash = '-' * 80
    print('{:{}s} {:>{}s} {:>{}s} {:>{}s}'.format('Benchmark', c1, 'Avg Time(μs)', c2, 'Peak Time(μs)', c3,
                                                  'Iterations', c4))
    print(dash)
    for output in outputs:
        print('{:{}s} {:15.0f} {:15.0f} {:14d}'.format(output[0], c1, float(output[1]), float(output[2]),
                                                       int(output[3])))
    print(dash)
