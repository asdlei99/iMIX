...
def _convert_to_one_hot(expected, output):
  ...
  if output.squeeze().dim() != expected.squeeze().dim() and expected.dim() == 1:
    expected = torch.nn.functional.one_hot(
        expected.long(), num_classes=output.size(-1)).float()
  return expected
...
class Metrics:
  ...
  def __call__(self, sample_list, model_output, *args, **kwargs):
    ...
    registry.register(
        '{}.{}.{}'.format('metrics', sample_list.dataset_name, dataset_type),
        values)
...
class BaseMetric:
  ...
  def calculate(self, sample_list, model_output, *args, **kwargs):
    ...
    ``SampleList`` and a dict returned by model as output and returns back a
    float tensor/number indicating value for this metric.
...
class Accuracy(BaseMetric):
  ...
  **Key:** ``accuracy``
  ...
  def calculate(self, sample_list, model_output, *args, **kwargs):
    ...
    assert (output.dim() <=
            2), "Output from model shouldn't have more than dim 2 for accuracy"
    assert (expected.dim() <=
            2), "Expected target shouldn't have more than dim 2 for accuracy"
...
class RecallAt1(RecallAtK):
  """Calculate Recall@1 which specifies how many time the chosen candidate was
  rank 1.
...
class RecallAt5(RecallAtK):
  """Calculate Recall@5 which specifies how many time the chosen candidate was
  among first 5 rank.
...
class RecallAt10(RecallAtK):
  """Calculate Recall@10 which specifies how many time the chosen candidate was
  among first 10 ranks.
...
class ROC_AUC(BaseMetric):
  ...
  **Key:** ``roc_auc``
  ...
  def calculate(self, sample_list, model_output, *args, **kwargs):
    """Calculate ROC_AUC and returns it back. The function performs softmax on
    the logits provided and then calculated the ROC_AUC.
...
class AveragePrecision(BaseMetric):
  ...
  **Key:** ``ap``
  ...
  def calculate(self, sample_list, model_output, *args, **kwargs):
    """Calculate AP and returns it back. The function performs softmax on the
    logits provided and then calculated the AP.
    ...
    expected = sample_list['targets']
    expected = _convert_to_one_hot(expected, output)
    value = average_precision_score(expected.cpu(), output.cpu(),
                                    **self._sk_kwargs)
    return expected.new_tensor(value, dtype=torch.float)
...
class RecallAtPrecisionK(BaseMetric):
  ...
  threshold. Use `p_threshold` param to specify the precision threshold i.e. k.
  Accepts precision in both 0-1 and 1-100 format.
  ...
  **Key:** ``r@pk``
  ...
  def calculate(self, sample_list, model_output, *args, **kwargs):
    ...
    precision, recall, thresh = precision_recall_curve(expected.cpu(),
                                                       output.cpu())
