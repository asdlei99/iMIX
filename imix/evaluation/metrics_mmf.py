...
"""The metrics module contains implementations of various metrics used commonly
to understand how well our models are performing. For e.g. accuracy,
vqa_accuracy, r@1 etc.
...
import torch
from mmf.common.registry import registry  # T0DO(jinliang)
...
def _convert_to_one_hot(expected, output):
    ...
    if output.squeeze().dim() != expected.squeeze().dim() and expected.dim() == 1:
        expected = torch.nn.functional.one_hot(expected.long(), num_classes=output.size(-1)).float()
    return expected
        ...
        for metric in metric_list:
            ...
            if isinstance(metric, collections.abc.Mapping):
                if 'type' not in metric:
                    raise ValueError(f"Metric {metric} needs to have 'type' attribute " + 'or should be a string')
                ...
                if key in metrics:
                    raise RuntimeError(f"Metric with type/key '{metric_type}' has been defined more " +
                                       'than once in metric list.')
            else:
                if not isinstance(metric, str):
                    raise TypeError('Metric {} has inappropriate type' "'dict' or 'str' allowed".format(metric))
                metric_type = key = metric

            metric_cls = registry.get_metric_class(metric_type)
            if metric_cls is None:
                raise ValueError(f'No metric named {metric_type} registered to registry')
    ...
    def __call__(self, sample_list, model_output, *args, **kwargs):
        ...
        with torch.no_grad():
            for metric_name, metric_object in self.metrics.items():
                ...
                values[key] = metric_object._calculate_with_checks(sample_list, model_output, *args, **kwargs)
        ...
        registry.register('{}.{}.{}'.format('metrics', sample_list.dataset_name, dataset_type), values)
...
class BaseMetric:
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        """Abstract method to be implemented by the child class. Takes in a
        ``SampleList`` and a dict returned by model as output and returns back
        a float tensor/number indicating value for this metric.
...
class Accuracy(BaseMetric):
    ...
    **Key:** ``accuracy``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        assert (output.dim() <= 2), "Output from model shouldn't have more than dim 2 for accuracy"
        assert (expected.dim() <= 2), "Expected target shouldn't have more than dim 2 for accuracy"
...
class VQAEvalAIAccuracy(BaseMetric):
    """Calculate Eval AI VQAAccuracy. Find more information here_ This is more
    accurate and similar comparision to Eval AI but is slower compared to
    vqa_accuracy.
            ...
            for answer_id in pred_answers[idx].tolist():
                if answer_id >= answer_space_size:
                    ...
                    if answer_id == answer_processor.EOS_IDX:
                        break
                    answer_words.append(answer_processor.answer_vocab.idx2word(answer_id))
...
class F1(BaseMetric):
    """Metric for calculating F1. Can be used with type and params argument for
    customization. params will be directly passed to sklearn f1 function.
...
class ROC_AUC(BaseMetric):
    """Metric for calculating ROC_AUC. See more details at
    `sklearn.metrics.roc_auc_score <http://scikit-learn.org/stable/modules/gene
    rated/sklearn.metrics.roc_auc_score.html#sklearn.metrics.roc_auc_score>`_ #
    noqa.
...
class AveragePrecision(BaseMetric):
    """Metric for calculating Average Precision. See more details at
    `sklearn.metrics.average_precision_score <http://scikit-learn.org/stable/mo
    dules/generated/sklearn.metrics.average_precision_score.html#sklearn.metric
    s.average_precision_score>`_ # noqa If you are looking for binary case,
    please take a look at binary_ap.
    ...
    **Key:** ``ap``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        """Calculate AP and returns it back. The function performs softmax on
        the logits provided and then calculated the AP.
...
class BinaryAP(AveragePrecision):
    """Metric for calculating Binary Average Precision. See more details at
    `sklearn.metrics.average_precision_score <http://scikit-learn.org/stable/mo
    dules/generated/sklearn.metrics.average_precision_score.html#sklearn.metric
    s.average_precision_score>`_ # noqa.
    ...
    **Key:** ``binary_ap``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        """Calculate Binary AP and returns it back. The function performs
        softmax on the logits provided and then calculated the binary AP.
...
class RecallAtPrecisionK(BaseMetric):
    """Metric for calculating recall when precision is above a particular
    threshold. Use `p_threshold` param to specify the precision threshold i.e.
    k. Accepts precision in both 0-1 and 1-100 format.
    ...
    **Key:** ``r@pk``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        try:
            value, _ = max((r, p) for p, r in zip(precision, recall) if p >= self.p_threshold)
        except ValueError:
            value = 0
