...
class Evaluator(PostProcessor):
  ...
  def __init__(self, metrics, dataset_converters):
    ...
    self._dataset_converters: list = self.build_dataset_converters(
        ...
        default_args={'post_process_type': str(self)})
    self._labels: list = []
  ...
  @classmethod
  def build_dataset_converters(cls, dataset_converters,
                               default_args: Optional[Dict]):
    dataset_converters = dataset_converters if isinstance(
        dataset_converters, list) else [dataset_converters]
...
class Submitter(PostProcessor):
  ...
  def __init__(self,
    ...
    post_process_type = {
        'post_process_type': str(self)
    } if post_process_type is None else post_process_type
  ...
  def process(self, *args, **kwargs):
    ...
    with open(self._save_file_name, 'w') as f:
      ...
      self.logger.info('The submit file has been saved to {}'.format(
...
class Predictor(Submitter):
  ...
  def __init__(self,
               ...
               *,
               output_dir: str = None,
               file_name: str = 'predict_result.pkl'):
    super().__init__(
        dataset_converters=dataset_converters,
  ...
  def process(self, *args, **kwargs):
    ...
    with open(self._save_file_name, 'wb') as f:
      ...
      self.logger.info('The prediction file has been saved to path {}:'.format(
          self._save_file_name))
...
def inference_on_dataset(cfg, model, data_loader):
  ...
  with to_inference(model), torch.no_grad():
    for idx, batch_data in enumerate(data_loader, start=1):
      logger.info('{} running idx: {}/{}'.format(
          str(post_processor), idx, len(data_loader)))
