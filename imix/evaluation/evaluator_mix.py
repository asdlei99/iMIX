        ...
        for evaluator in self._evaluators:
            ...
            if is_main_process() and result is not None:
                for key, value in result.items():
                    if key in results:
                        raise KeyError('This evaluator {} is already in results'.format(key))
                    else:
                        results[key] = value
        return results
...
def inference_on_dataset(model, data_loader, evaluator):
    ...
    with to_inference(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader, start=1):
            ...
            single_infer_time = Timer.passed_seconds(start=start_infer_time, end=Timer.now())
            total_inference_time += single_infer_time
            evaluator.eval_process(inputs, outputs)
            if idx % 10 == 0 or idx == num_imgs:
                total_seconds_per_img = total_inference_time / idx
                eta_time = datetime.timedelta(seconds=int(total_seconds_per_img * (num_imgs - idx)))
                logger.info('Inference completion ratio {}/{} {:.4f}s/img, ETA:{}'.format(
                    idx, num_imgs, single_infer_time, eta_time))
    ...
    infer_log_msg = 'Total inference time:{} speed:{:.6f} s/img per device, on {} devices'.format(
        total_inference_time, total_inference_time / (num_imgs * world_size), world_size)
...
def build_test_predict_result(model, data_loader):
    ...
    with to_inference(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader, start=1):
            ...
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            for qid, pred in zip(inputs['question_id'].detach().numpy(), outputs['scores'].cpu().detach().numpy()):
                test_predict['question_id'] = qid
                test_predict['scores'] = pred

    if is_main_process():
        logger.info('vqa data size:{}'.format(len(test_predict)))
        save_path = '/home/jinliang/code/imix/imix/work_dir/inference/vqa_test_predict.pkl'
