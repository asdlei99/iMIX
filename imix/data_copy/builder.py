...
from imix.utils_mix.registry import Registry, build_from_cfg
...
def build_dataloader(dataset,
  ...
  if dist:
    ...
    sampler = GroupSampler(
        dataset, samples_per_gpu
    ) if shuffle else None  # TODO(jinliang) mmdet:flag according to image aspect ratio
...
def build_data_loader_by_iter(dataset, cfg, is_training=True):
  ...
  if is_training:
    ...
    batch_sampler = BatchSampler(
        sampler, cfg.test_data.samples_per_gpu, drop_last=False)
    data_loader = DataLoader(
        dataset,
        num_workers=num_workers,
        batch_sampler=batch_sampler,
