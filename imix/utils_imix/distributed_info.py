...
def _get_global_gloo_group():  # TODO(jinliang):COPY
  """Return a process group based on gloo backend, containing all the ranks The
  result is cached."""
...
def _serialize_to_tensor(
    data, group):  # TODO(jinliang):serialize2tensor -> is_serialize ???
...
def _pad_to_largest_tensor(tensor: torch.Tensor,
                           group) -> tuple:  # TODO(jinliang):alignment_tensor
  """
        Returns:
  ...
  if world_size < 1:
    raise Exception(
        'dist.gather/all_gather must be called from ranks with in the given group',
        world_size)
  ...
  tensor_sizes = [
      torch.zeros([1], dtype=dtype, device=device) for _ in range(world_size)
  ]
...
def all_gather(data, group=None) -> list:
  ...
  max_tensor_size = max(tensor_sizes)
  tensor_list = [
      torch.empty((max_tensor_size,), dtype=torch.uint8, device=tensor.device)
      for _ in tensor_sizes
  ]
...
def gather(data,
           *,
           dst_rank=0,
           group=None) -> list:  # TODO(jinliang): gather --> gather_data
...
def shared_random_seed(low=2**31,
                       select_idx=0
                      ) -> int:  # TODO(jinliang): get_common_random_seed
  """
        Returns:
