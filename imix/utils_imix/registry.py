...
class Registry:
  """Registry class: mapping to the corresponding object based on the provided
  name."""
  ...
  def _register_module(self, module_name: str, module_class: object) -> None:
    assert (inspect.isclass(module_class)
           ), 'module must be a class, but the input module type is {}'.format(
               type(module_class))
    assert (module_name not in self._module_dict
           ), 'A module named {} was already registered in {}'.format(
  ...
  def register_module(self,
                      name: str = None,
                      module: object = None) -> Optional[object]:
...
def build_from_cfg(cfg: Dict,
  ...
  if isinstance(object_type, str):
    ...
    raise TypeError(
        'The type in cfg must be a string or class type, but got{}'.format(
            type(object_type)))
