...
class YOLOLoss(BaseLoss):
  ...
  def forward(self, predict_anchor, target_bbox):
    losses = YOLOLoss.compute_loss(predict_anchor, target_bbox,
                                   self.anchors_full)
    return losses
  ...
  def calculate_loss(input,
    ...
    for ii in range(batch):
      pred_bbox[ii, 0:2] = F.sigmoid(
          input[best_n_list[ii] // 3][ii, best_n_list[ii] % 3, 0:2, gj[ii],
                                      gi[ii]])
      pred_bbox[ii, 2:4] = input[best_n_list[ii] // 3][ii, best_n_list[ii] % 3,
                                                       2:4, gj[ii], gi[ii]]
      gt_bbox[ii, :] = target[best_n_list[ii] // 3][ii, best_n_list[ii] % 3, :4,
                                                    gj[ii], gi[ii]]
    ...
    for scale_ii in range(len(input)):
      pred_conf_list.append(input[scale_ii][:, :, 4, :, :].contiguous().view(
          batch, -1))
      gt_conf_list.append(target[scale_ii][:, :, 4, :, :].contiguous().view(
          batch, -1))
  ...
  def compute_loss(pred_anchor_list, bbox, anchors_full):
    gt_param, gi, gj, best_n_list = YOLOLoss.build_target(
        bbox, pred_anchor_list, anchors_full)
    ...
    ## loss
    loss = YOLOLoss.calculate_loss(pred_anchor_list, gt_param, gi, gj,
                                   best_n_list)
    return loss
    # losses = {'loss': loss}
