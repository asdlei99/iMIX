...
class VisualDialogueHead(
    nn.Module,
    metaclass=ABCMeta):  # VISDIALPRINCIPLES_HEAD  reanme --> VisualDialogueHead
...
class DiscQtDecoderHead(VisualDialogueHead):
  ...
  def forward(self, encoder_output, batch):
    """Given `encoder_output` + candidate option sequences, predict a score for
    each option sequence.
    ...
    _, (options_feat, _) = self.option_rnn(options_embed,
                                           options_length)  # b*100 512
...
class DiscByRoundDecoderHead(VisualDialogueHead):
  ...
  def __int__(self, vocabulary_len, word_embedding_size, lstm_hidden_size,
              lstm_num_layers, dropout, *args, **kwargs):
    super().__init__(*args, **kwargs)

    self.nhid = lstm_hidden_size

    self.word_embed = nn.Embedding(
        vocabulary_len, word_embedding_size, padding_idx=0)
    self.option_rnn = nn.LSTM(
        word_embedding_size,
        lstm_hidden_size,
        lstm_num_layers,
    ...
    self.a2a = nn.Linear(self.nhid * 2,
                         self.nhid)  # this is useless in this version
...
@HEADS.register_module()
class LanguageDecoderHead(
    nn.Module):  # VISDIALPRINCIPLES_HEAD  reanme --> VisualDialogueHead
