...
class VISDIALPRINCIPLES_BACKBONE(nn.Module):
  ...
  def __init__(self, dropout, nhid, ninp, img_feature_size, dropout_fc):
    ...
    self.q_ref = nn.Sequential(
        nn.Dropout(p=self.dropout_fc), nn.Linear(self.nhid * 2, self.nhid),
        nn.LeakyReLU(), nn.Dropout(p=self.dropout_fc), nn.Linear(self.nhid, 2),
        nn.LeakyReLU())
    self.q_multi = nn.Sequential(
        nn.Dropout(p=self.dropout_fc), nn.Linear(self.nhid * 2, self.nhid),
        nn.LeakyReLU(), nn.Dropout(p=self.dropout_fc), nn.Linear(self.nhid, 3),
        nn.LeakyReLU())
  ...
  def q_att_on_cap(self, ques_prin, cap_prin, cap_feat, cap_len, cap_emb):
    ...
    cap_score = self.Wall3(
        self.dropout(
            torch.tanh(cap_feat_new + q_emb.repeat(1, capfeat_len, 1) +
    ...
    final_cap_feat = torch.bmm(weight.view(batch_size, 1, -1),
  ...
  def c_att_on_ques(self, cap_prin, ques_prin, ques_feat, ques_len, ques_emb):
    ...
    ques_score = self.Wall5(
        self.dropout(
            torch.tanh(ques_feat_new + q_emb.repeat(1, quesfeat_len, 1) +
                       c_emb.repeat(1, quesfeat_len, 1)))).view(batch_size, -1)
    mask = ques_score.detach().eq(0)
    for i in range(batch_size):
      mask[i, ques_len[i]:] = 1
    ques_score.masked_fill_(mask, -1e5)
    weight = torch.softmax(ques_score, dim=-1)
    final_ques_feat = torch.bmm(weight.view(batch_size, 1, -1),
  ...
  def q_att_on_img(self, ques_feat, img_feat):
    ...
    all_score = self.Wall2(
        self.dropout(torch.tanh(i_emb * q_emb.repeat(1, region_size, 1)))).view(
            batch_size, -1)
    img_final_feat = torch.bmm(
        torch.softmax(all_score, dim=-1).view(batch_size, 1, -1), img_feat)
  ...
  def c_att_on_img(self, cap_feat, img_feat):
    ...
    all_score = self.Wall4(
        self.dropout(torch.tanh(i_emb * c_emb.repeat(1, region_size, 1)))).view(
            batch_size, -1)
    img_final_feat = torch.bmm(
        torch.softmax(all_score, dim=-1).view(batch_size, 1, -1), img_feat)
    return img_final_feat.view(batch_size, -1)
  ...
  def ques_att_on_his(self, ques_feat, his_feat):
    ...
    score = self.Wqh1(
        self.dropout(torch.tanh(h_emb + q_emb.repeat(1, rnd, 1)))).view(
            batch_size, -1)
  ...
  def forward(self, ques_encoded, cap_encoded, his_feat, q_output, c_output,
              ques_len, cap_len, ques_embed, cap_emb, img, batch_size):

    q_att_his_feat = self.ques_att_on_his(ques_encoded,
                                          his_feat).view(batch_size,
                                                         self.nhid)  # b 512
    att_cap_feat_0 = self.q_att_on_cap(ques_encoded, cap_encoded, c_output,
                                       cap_len, cap_emb)  # (batch_size, 2*nhid)
    att_ques_feat_0 = self.c_att_on_ques(cap_encoded, ques_encoded, q_output,
                                         ques_len, ques_embed)
    his_att_cap = self.q_att_on_cap(q_att_his_feat, cap_encoded, c_output,
                                    cap_len, cap_emb)
    ...
    q_att_img_feat = self.q_att_on_img(att_ques_feat,
                                       img).view(batch_size, -1,
                                                 self.img_feature_size)
    # attended cap on img
    c_att_img_feat = self.c_att_on_img(att_cap_feat,
                                       img).view(batch_size, -1,
                                                 self.img_feature_size)
    cated_feature = torch.cat((q_att_img_feat, c_att_img_feat), dim=1)
    q_gs = torch.softmax(
        (self.q_ref(torch.cat((att_ques_feat_0, his_att_cap), dim=-1))),
    ...
    fused_vector = torch.cat(
        (att_ques_feat, final_img_feat, ques_encoded * img_feat_fusion), dim=-1)
    ...
    fused_embedding = torch.cat(
        (fused_embedding_1, fused_embedding_2, fused_embedding_3), dim=1)
    q_multi = torch.softmax((self.q_multi(att_ques_feat)),
                            dim=-1).view(batch_size, 1, -1)
