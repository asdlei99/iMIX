...
class HGL_BACKBONE(R2C_BACKBONE):
  ...
  def forward(self, box_mask, a_rep, q_rep, obj_reps, question_mask,
              answer_mask):
    ...
    qa_attention_weights = masked_softmax(
        qa_similarity, question_mask[..., None], dim=2)
    ...
    atoo_attention_weights = masked_softmax(atoo_similarity, box_mask[:, None,
                                                                      None])
    attended_o = torch.einsum('bnao,bod->bnad',
                              (atoo_attention_weights, obj_reps['obj_reps']))

    a_rep, attended_o, attended_q = self.Graph_reasoning(
        a_rep, attended_o, attended_q)

    reasoning_inp = torch.cat([
        x for x, to_pool in [(
            a_rep, self.reasoning_use_answer
        ), (attended_o,
            self.reasoning_use_obj), (attended_q, self.reasoning_use_question)]
        if to_pool
    ], -1)
    ...
    things_to_pool = torch.cat([
        x for x, to_pool in [(reasoning_output,
                              self.pool_reasoning), (a_rep, self.pool_answer),
                             (attended_q, self.pool_question)] if to_pool
    ], -1)

    pooled_rep = replace_masked_values(things_to_pool.float(),
                                       answer_mask[...,
                                                   None].bool(), -1e7).max(2)[0]
...
class Graph_reasoning(nn.Module):
  ...
  def forward(self, answer, o_a, q_a):
    ...
    A_obj = F.softmax(
        self.w_g_o(F.relu(self.w_s_o(s_obj) + self.w_s_o_(e_obj))), dim=-2)
    A_ans = F.softmax(
        self.w_g_a(F.relu(self.w_s_a(s_ans) + self.w_s_a_(e_ans))), dim=-2)
    A_que = F.softmax(
        self.w_g_q(F.relu(self.w_s_q(s_que) + self.w_s_q_(e_que))), dim=-2)
