...
class CAGRAPH_BACKBONE(nn.Module):
  ...
  def forward(self, ques_feat, his_feat, rcnn_feat, ques_emb, rnd):
    ...
    his_atten_weight = F.softmax(
        self.Wa_1(
            F.dropout(atten_emb_1, self.d,
                      training=self.training).view(-1,
                                                   self.nhid)).view(-1, rnd))
    h_emb = torch.bmm(
        his_atten_weight.view(-1, 1, rnd), his_feat.view(-1, rnd, self.nhid))
    ...
    at = F.softmax(
        self.Wqt(
            F.dropout(q_norm, self.d,
                      training=self.training).view(-1, self.nhid)).view(-1, L))
  ...
  def _create_neighbourhood(self, message, belief_matrix, neighbourhood_size):
    ...
    top_k = torch.stack([F.softmax(top_k[:, k]) for k in range(K)
                        ]).transpose(0,
