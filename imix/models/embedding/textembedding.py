...
class TextEmbedding(nn.Module):
  ...
  def __init__(self,
    ...
    if emb_type == 'identity':
      ...
      self.module = BiLSTMTextEmbedding(hidden_dim, embedding_dim, num_layers,
                                        dropout, **kwargs)
      ...
      self.module = AttentionTextEmbedding(hidden_dim, num_layers, conv1_out,
                                           conv2_out, dropout, embedding_dim,
                                           kernel_size, padding, **kwargs)
      self.text_out_dim = self.module.text_out_dim

    elif emb_type == 'mcan':
      self.module = SAEmbedding(hidden_dim, embedding_dim, num_layers, dropout,
                                **kwargs)
...
class _netW(nn.Module):
  ...
  def init_pretrained_wemb(self, emb_dim):
    """From blog.keras.io Initialises words embeddings with pre-trained GLOVE
    embeddings."""
