...
class R2C(BaseModel):
  ...
  def __init__(self, input_dropout, pretrained, average_pool, semantic,
    ...
    self.encoder_model = TimeDistributed(
        PytorchSeq2SeqWrapper(
            torch.nn.LSTM(1280, 256, batch_first=True, bidirectional=True)))
  ...
  def _collect_obj_reps(self, span_tags, object_reps):
    ...
    return object_reps[row_id.view(-1),
                       span_tags_fixed.view(-1)].view(*span_tags_fixed.shape,
                                                      -1)
...
class SimpleDetector(nn.Module):
  ...
  def __init__(self,
    ...
    self.roi_align = RoIAlign(
        (7, 7) if USE_IMAGENET_PRETRAINED else (14, 14),
        spatial_scale=1 / 16,
        sample_num=0)

    if semantic:
      self.mask_dims = 32
      self.object_embed = torch.nn.Embedding(
          num_embeddings=81, embedding_dim=128)
