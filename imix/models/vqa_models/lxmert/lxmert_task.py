from imix.models.builder import VQA_MODELS, build_backbone, build_embedding, build_encoder, build_head, build_combine_layer
...
class LXMERT(BaseModel):
  ...
  def __init__(self, **kwargs):
    ...
    if training_head_type == 'pretraining':
      ...
      if training_head_type == 'vqa2':
        ...
        raise KeyError(
            'NO head type support for {}!'.format(training_head_type))
  ...
  def forward_train_pretrain(self, data):
    ...
    if params.get("feats") is not None and params.get("image_dim") is not None:
      image_mask = (
          torch.arange(
              params["feats"].size(-2)).expand(*params["feats"].size()[:-1]))
    ...
    output_dict = self.model(
        ...
        if params["visual_attention_mask"] is not None else
        params["visual_attention_mask"],
    )
    target_dict = {
        ...
        "ans": params["ans"].cuda(),
        "obj_labels": {
            "obj": (params["det_obj_labels"].cuda(),
                    params["det_obj_confs"].cuda()),
            "attr": (params["det_attr_labels"].cuda(),
                     params["det_attr_confs"].cuda()),
