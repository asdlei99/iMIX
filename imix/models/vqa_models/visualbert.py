...
class VisualBERT(BaseModel):
  ...
  def flatten(self, data, to_be_flattened=None, to_be_flattened_dim=None):
    ...
    if data['visual_embeddings_type'] is None:
      if data['image_mask'] is not None:
        data['visual_embeddings_type'] = torch.zeros_like(
            data['image_mask'], dtype=torch.long)

    if data['image_mask'] is not None:
      attention_mask = torch.cat((data['input_mask'], data['image_mask']),
                                 dim=-1)
      if data['input_lm_label_ids'] is not None:
        assert data['input_lm_label_ids'].size(-1) == data['input_mask'].size(
            -1)
        ...
        new_lm_labels[:size_masked_lm_labels[0], :
                      size_masked_lm_labels[1]] = data['input_lm_label_ids']
  ...
  def add_custom_params(self, data):
    ...
    if visual_embeddings is not None:
      image_mask = (
          torch.arange(visual_embeddings.size(-2)).expand(
  ...
  def format_state_key(cls, key):
    return (key.replace('bert.bert', 'model.bert').replace(
        'bert.cls', 'model.cls').replace('bert.classifier', 'model.classifier'))
