...
Pytorch modules some classes are modified from HuggingFace (https://github.com/huggingface/transformers)
"""
import copy
import json
import logging
...
class UniterConfig(object):
    ...
    def from_dict(cls, json_object):
        """Constructs a `UniterConfig` from a Python dictionary of
        parameters."""
...
class UniterPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, UniterConfig):
            raise ValueError('Parameter config in `{}(config)` should be an instance of '
