...
class ImageFeatureEncoder(nn.Module):
  ...
  def __init__(self,
    ...
    if encoder_type == 'default':
      ...
      self.module = FinetuneFasterRcnnFpnFc7(in_dim, weights_file, bias_file,
                                             **kwargs)
    else:
...
def parse_model_config(path):
  ...
  for line in lines:
    if line.startswith('['):  # This marks the start of a new block
      ...
      if module_defs[-1]['type'] == 'convolutional' or module_defs[-1][
          'type'] == 'yoloconvolutional':
...
def create_modules(module_defs):
  ...
  for i, module_def in enumerate(module_defs):
    ...
    if module_def['type'] == 'convolutional' or module_def[
        'type'] == 'yoloconvolutional':
...
class YOLOLayer(nn.Module):
  ...
  def forward(self, x, targets=None):
    ...
    grid_x = torch.arange(nG).repeat(nG, 1).view([1, 1, nG,
                                                  nG]).type(FloatTensor)
    grid_y = torch.arange(nG).repeat(nG, 1).t().view([1, 1, nG,
                                                      nG]).type(FloatTensor)
    ...
    if targets is not None:
      ...
      loss_conf = self.bce_loss(
          pred_conf[conf_mask_false], tconf[conf_mask_false]) + self.bce_loss(
...
class DarknetEncoder(nn.Module):
  ...
  def load_weights(self, weights_path):
    ...
    if self.config_path == './model/yolo9000.cfg':
      header = np.fromfile(
          fp, dtype=np.int32, count=4)  # First five are header values
    else:
      header = np.fromfile(
