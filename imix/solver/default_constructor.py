...
from ..utils_imix.registry import build_from_cfg, Registry
...
class DefaultOptimizerConstructor:
  ...
  def _validate_cfg(self):
    ...
    if ('bias_decay_mult' in self.paramwise_cfg or
        'norm_decay_mult' in self.paramwise_cfg or
        'dwconv_decay_mult' in self.paramwise_cfg):
      if self.base_wd is None:
        raise ValueError('base_wd should not be None')
  ...
  def add_params(self, params, module, prefix=''):
    ...
    is_dwconv = (
        isinstance(module, torch.nn.Conv2d) and
